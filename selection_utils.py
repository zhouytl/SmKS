...
def auto_select(evtdir, fn_measure, objlist):

    df_measure = pd.read_csv('%s/%s' % (evtdir, fn_measure))

    # objdir0 = '%s/Figures/individual_0' % evtdir
    # objdir1 = '%s/Figures/synchronized_1' % evtdir
    # objdir2 = '%s/Figures/synchronized_2' % evtdir

    for objdir in objlist:
        ...
        for idx, row in df_measure.iterrows():
            ...
            if row.snr < 2.0 or row.ccf1 < 0.3 or row.ccf2 < 0.3:
...
def group_traces(fout, fname, figdir):
